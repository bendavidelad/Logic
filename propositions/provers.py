...
def inverse_mp(proof, assumption):
    ...
    for line in range(len(proof.lines)):
        if proof.lines[line].conclusion == assumption:
            ...
            current_lines_to_add = lines_for_other_assumption_or_i1_or_i2(proof.lines[line], assumption,
                                                                          len(new_proof_lines))
            new_proof_lines.extend(current_lines_to_add)
            assumptions_dict[line] = len(new_proof_lines) - 1
        else:
            current_lines_to_add = lines_for_mp_rule(proof.lines[line], assumption, proof,
                                                     len(new_proof_lines), assumptions_dict)
            new_proof_lines.extend(current_lines_to_add)
            assumptions_dict[line] = len(new_proof_lines) - 1
    ...
    return DeductiveProof(InferenceRule(
        updated_assumptions, new_proof_lines[len(new_proof_lines) - 1].conclusion), proof.rules,
        new_proof_lines)


def lines_for_the_given_assumption(assumption):
    proved_implies_self = prove_implies_self()
    proved_instance = prove_instance(proved_implies_self,
                                     InferenceRule([], Formula('->', assumption, assumption)))
...
def lines_for_other_assumption_or_i1_or_i2(line, assumption, line_number):
    ...
    line2 = DeductiveProof.Line(Formula("->", line1.conclusion, Formula("->", assumption, line1.conclusion)),
                                1, [])
    line3 = DeductiveProof.Line(Formula("->", assumption, line1.conclusion), 0,
...
def lines_for_mp_rule(line, assumption, proof, line_number, assumptions_dict):
    ...
    line1 = DeductiveProof.Line(Formula("->", Formula("->", assumption, Formula("->", first_assumption,
                                                                                conclusion)),
                                        Formula("->", Formula("->", assumption,
                                                              first_assumption),
                                                Formula("->", assumption, conclusion))), 2, [])
