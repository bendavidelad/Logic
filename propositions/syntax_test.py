...
def test_infix(debug=False):
    ...
    assert Formula('~', Formula('&', Formula('p'),
                                Formula('q7'))).infix() == '~(p&q7)'
...
def test_infix_all_operators(debug=False):
    for binary_operator in ['->', '<->', '-&', '-|']:
        ...
        assert Formula(binary_operator, Formula('p'),
                       Formula('q')).infix() == infix

    if debug:
        print("Testing infix of formula '(p?q:r)'")
    assert Formula('?:', Formula('p'), Formula('q'),
                   Formula('r')).infix() == '(p?q:r)'
...
def test_prefix_all_operators(debug=False):
    for binary_operator in ['->', '<->', '-&', '-|']:
        ...
        if debug:
            print(
                "Testing prefix of formula '" + prefix + "' (in infix: '" + infix + "')")
        assert Formula(binary_operator, Formula('p'),
                       Formula('q')).prefix() == prefix

    if debug:
        print("Testing prefix of formula '?:pqr' (in infix: '(p?q:r)')")
    assert Formula('?:', Formula('p'), Formula('q'),
                   Formula('r')).prefix() == '?:pqr'


def test_from_prefix_all_operators(debug=False):
    for prefix in [ '->~->xyz', '~?:TTT',
                   '?:<->xxxx', '?:F-&y123F-|y123F']:
        if debug:
            print("Testing prefix parsing of formula", prefix)
